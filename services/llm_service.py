...
from consts import TAIGA_USER_NAME, TEMP_DIR
...
class LLMService:
    ...
    def generate_prompt_for_task_create(self, text: str, limit: int) -> list[dict]:
        ...
        with open(f"{TEMP_DIR}/messages_task_create.txt", "w", encoding="utf-8") as f:
    ...
    def generate_prompt_for_task_change(self, subject: str, comments: list[dict], last_comment: dict, limit: int=4) -> list[dict]: 
        ...
        with open(f"{TEMP_DIR}/messages_task_change.txt", "w", encoding="utf-8") as f:
            f.write(str(messages))
        return messages